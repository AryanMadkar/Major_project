#!/usr/bin/env python3
"""Process markdown files in multi_property_dataset/sale using the phase0 splitter.

Creates a `responses_sales/` directory under the `bullshit` folder and writes a JSON file
and a plain-text file for each input markdown containing the extracted
single-line property messages.

Usage:
    python process_sale_dataset.py
"""
...
def main():
    ...
    dataset_dir = repo_root / "multi_property_dataset" / "sale"
    if not dataset_dir.exists():
        print("Dataset directory not found:", dataset_dir)
        sys.exit(1)

    out_dir = repo_root / "responses_sales"
