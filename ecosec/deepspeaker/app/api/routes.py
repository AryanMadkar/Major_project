...
from app.utils.validators import validate_extension, validate_file_size
...
def verify():
    ...
    with Timer("Verification Request"):
        ...
        try:
            ...
            logger.info(f"[{request_id}] Verification request started")
            ...
            logger.info(f"[{request_id}] Verification request completed in {latency:.4f} seconds")
            ...
            logger.warning(f"[{request_id}] Verification request rejected: {exc}")
            return error_response(str(exc), status=400)
